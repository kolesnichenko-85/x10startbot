...
function addDust(root){if(USE_FRAME_RENDERER)return;for(let i=0;i<26;i++){const p=document.createElement('i');p.style.left=(38+Math.random()*24)+'%';p.style.top=(39+Math.random()*18)+'%';root.appendChild(p);setTimeout(()=>{try{p.animate?.([{opacity:0,transform:'translate(0,0) scale(.2)'},{opacity:.9,transform:`translate(${(Math.random()-.5)*130}px,${-30-Math.random()*100}px) scale(1)`},{opacity:0,transform:`translate(${(Math.random()-.5)*220}px,${-100-Math.random()*170}px) scale(.2)`}],{duration:1000+Math.random()*700,easing:'ease-out',fill:'forwards'})}catch(e){}},1120+Math.random()*220)}}
...
function showRevealV6(item){
 ...
 const creatureMarkup=(model&&!USE_FRAME_RENDERER)
